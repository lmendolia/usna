/*
        ...
        Resources: Alex Varon, Adam Montgomery, Katie Swafford, MGSP
...
int main( int argc, char *argv[] ) {
        ...
        while ((opt = getopt(argc, argv, ":p:h")) != -1){
                if (opt == '?' || opt == ':'){
                        printf("; optopt = %3d (%c)"opt, printable(optopt), optind);}
                printf("\n");
        switch(opt) {
        ...
        if (optind < argc){
                printf("First argument is \"%s\" at argv[%d]\n",
                argv[optind], optind);
                        exit(0);
                        }
                }
        ...
        exit(0);//EXIT WITH NORMAL RETURN VALUE
                ...
                /*  if( divisor == 0){
                   ...
                   ^^this snippet was taken from tutorialspoint.com --> looking at it for syntax
                   */
